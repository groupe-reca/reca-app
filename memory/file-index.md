...
- schemas: `src/features/clients/schemas/client.schema.ts` (tâche 5 — `CLIENT_TYPES`/`ClientType` enum `residentiel`|`commercial` remplace le `typeClient` free-text, `entreprise` requis si commercial via `.refine()`)
