...
- components: `ContractFormModal.tsx`, `ContractForm.tsx`, `ContractStatusBadge.tsx`, `ContractTable.tsx`, `PaymentScheduleBuilder.tsx`, `TextareaField.tsx`, `ContractWizardDefaultsForm.tsx` (nouveau, tâche 5 — formulaire des paramètres par défaut du Wizard), `ContractSummaryStats.tsx` (nouveau, tâche 8 — cartes label/valeur "Informations du contrat" de `ContractCreatedPage`, `StatCard` essayé puis écarté car pensé pour des KPI numériques courts). **`ContractBasicInfoFields.tsx` supprimé (tâche 5)** — plus aucun champ à porter, tous dérivés/déplacés.
- components/contract-document (nouveau, tâche 8 — prévisualisation HTML "façon future page PDF" affichée sur `ContractCreatedPage`, composants **purs**, aucun hook/router à l'intérieur, réutilisables tels quels par un futur moteur PDF) : `types.ts` (`ContractDocumentData`), `DocumentSectionHeader.tsx` (primitif d'en-tête `plain`/`navy`), `DocumentHeader.tsx`, `DocumentServicesModalites.tsx`, `DocumentClientZones.tsx`, `DocumentClausesList.tsx`, `DocumentSummaryPayment.tsx`, `DocumentSignatures.tsx`, `DocumentFooterBar.tsx`, `ContractDocumentPreview.tsx` (orchestrateur racine). Grilles internes en `sm:` (640px, jamais `lg:`) — voir note piège dans `memory/memory.md`.
