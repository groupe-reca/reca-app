...
**Question posée à l'utilisateur avant implémentation** : quelle profondeur pour "Historique du contrat", qui nécessite une vraie infra d'événements (explicitement exclue à la tâche 9) — réponse : **vrai journal d'événements léger** (nouvelle table `contract_events`), pas une maquette statique ; chaque ligne affichée doit correspondre à une action réelle (création, changement de statut, clic réel sur "Envoyer par courriel"/"Télécharger PDF", changement réel de date de signature), aucune donnée fabriquée. Déviation assumée par rapport à la maquette : l'acteur de "PDF généré"/"Courriel envoyé" sera l'utilisateur connecté qui a cliqué (toujours un admin, ces actions restent des placeholders synchrones), pas "Système" comme illustré (aucun job asynchrone réel n'existe).

**Plan détaillé complet** (table `contract_events` + RLS + FK users dès la migration, nouveaux fichiers `contractEvent.types.ts`/`contractEvents.service.ts`/`useContractEvents.ts`/`useLogContractEvent.ts`/`ContractEventTimelineItem.tsx`/`ContractHistoryCard.tsx`/`ContractHistoryModal.tsx`/`ContractClausesModal.tsx`, points d'écriture réels dans `contracts.service.ts`/`useUpdateContractStatus.ts`/`ContractFormModal.tsx`/pages détail, restyle de `Button.tsx` (variant `info` bleu)/`ContractDetailHeader.tsx`/`ContractInfoStrip.tsx`/`ContractClientCard.tsx`/`ContractMapCard.tsx`/`ContractOperatorInfoCard.tsx`/`ContractClausesCard.tsx`/`ContractPaymentsCard.tsx` (tableau inline, `ContractPaymentsHistoryModal.tsx` supprimé après), réassemblage des 2 pages détail) : `/root/.claude/plans/jiggly-snacking-cocke.md`.
...
## [x] Module Routes — architecture complète (branche `conception-du-module-routes`, depuis `main`, implémenté et vérifié 2026-07-21)

Plan détaillé complet (contexte, 8 phases, fichiers, vérification) : `/root/.claude/plans/flickering-drifting-gray.md`.

Résumé : le module Routes livré initialement (commit `af9f848`) n'était qu'un squelette CRUD manuel ("en planification"). Doc06 (MODULE G, "le cœur du logiciel") attend 3 **Vues** (Carte/Liste/Timeline) et 4 **Actions** (Démarrer/Terminer/Modifier/Imprimer) qui manquaient toutes. Décisions validées avec l'utilisateur avant le plan : (1) les 3 vues vivent sur `RoutesListPage` (bascule de niveau page, pas onglets sur la fiche détail) ; (2) distance/durée estimée passent d'une saisie manuelle à un **calcul automatique via l'API Mapbox Directions** à partir des coordonnées GPS déjà stockées sur les clients (repli manuel si Mapbox non configuré, même convention qu'`AddressAutocomplete.tsx`). Nouvelle colonne `routes.trace_geojson` (migration `20260720000000_routes_trace_geojson.sql`, à appliquer manuellement par l'utilisateur comme toutes les précédentes) pour persister le tracé et éviter de rappeler Directions à chaque rendu de la vue Carte. Réutilise `useMapboxMap`/`MapCanvas` (génériques, déjà construits pour Contrats) et l'infra PDF partagée (pattern `generateXPdf.tsx`/`XPdfDocument.tsx`/`sections/`, import dynamique obligatoire). Deux ajouts au-delà du spec littéral, à signaler à l'utilisateur : menu secondaire Suspendre/Reprendre (le statut "Suspendue" existe dans l'énum mais n'a pas d'action dédiée dans le doc) et indicateur de conflit sur la vue Timeline (double assignation employé+jour, détectable seulement en croisant `route_assignments` puisque les employés n'ont pas de champ de disponibilité). Hors scope explicite : intégration Dashboard, expérience mobile dédiée, `intervention_logs`/Application employé.
  **Implémenté en entier (8 phases) et vérifié de bout en bout 2026-07-21, migration comprise** : `tsc -b`/`npm run lint`/`npm run build` propres (chunk `generateRoutePdf-*.js` bien code-splitté). `RouteDetailPage.tsx` refactorisé avec un nouveau `RouteDetailHeader.tsx` extrait (pattern `InvoiceDetailHeader.tsx`) pour rester sous ~300 lignes. **Test réel complet réussi** (Playwright, compte admin, port 3010 via pm2, 2 clients réels géocodés + 1 employé + 1 route créés puis supprimés après vérification) : calcul auto Mapbox Directions confirmé (1.48 km/6min11s entre 2 vraies adresses Montréal), vue Carte avec vrai tracé routier, Timeline avec vraie assignation, Démarrer/Terminer/Imprimer (PDF réel) tous fonctionnels, zéro erreur console. Modules Routes/Employés réactivés temporairement pour le test puis redésactivés (état Paramètres → Modules restauré identique). Détail complet : `memory/tasks.md` (section "Module Routes"). Le module est maintenant pleinement fonctionnel. Reste : commit (pas encore demandé) et réactivation permanente du module par l'utilisateur si Routes doit devenir visible aux utilisateurs.
