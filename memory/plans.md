...
**Vérification (2026-07-15)** : `tsc -b`/`npm run lint`/`npm run build` propres. Vérifié en navigateur (Playwright, compte admin, port 3011) : fiche client (sélecteur + Commercial révèle Entreprise + suggestions Mapbox réelles), `ClientSearchPicker` (badge type + adresse "4100 Rue Talon, Sainte-Catherine, QC J5C 1T1" + téléphone "(438) 220-4110" confirmés sur un client réel), icône ⚙️ → page Paramètres (état d'erreur propre, pas de spinner infini — colonne pas encore en live), Wizard étape 1 "Adresse Validée" verte + bouton "Outil de mesure" (ouvre directement la carte, zéro régression sur tracé/capture/zoom), **clic "Suivant" depuis l'étape 1 saute bien "Analyse & Zones" → atterrit directement sur "Modalités de paiement"** (barre de progression à 4 étapes), étape Paiement sans bloc Obligations, Prix présent, Échéancier à 2 boutons (Mensuel absent), Notes en bas de l'étape. Aucune donnée de test créée (lecture seule sur un client/contrat déjà existants — "Francis Bastien"/CTR-000005). **Les 2 migrations sprint014 pendantes + la nouvelle migration `contract_wizard_defaults` restent à appliquer par l'utilisateur avant que la création réelle/le brouillon/la page de paramètres soient pleinement fonctionnels de bout en bout.** Pas encore commité.
