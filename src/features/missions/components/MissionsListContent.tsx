...
import { CalendarDays, ChevronDown, ChevronUp, MapPin, Search, SlidersHorizontal, Truck, User } from 'lucide-react'
...
function MissionsListBody({
  ...
  const hasActiveFilters = Boolean(
    statusFilter !== 'tous' || dateFilter || routeFilter || operatorFilter || equipmentFilter,
  )
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Button variant="secondary" fullWidth className="sm:w-auto sm:self-start" onClick={() => setFiltersOpen((open) => !open)}>
        <SlidersHorizontal className="size-4" aria-hidden="true" />
        Filtres
        {hasActiveFilters && (
          <span className="size-1.5 rounded-full bg-reca-red" aria-label="Filtres actifs" />
        )}
        {filtersOpen ? (
